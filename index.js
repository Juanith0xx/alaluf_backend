...
// ─────────────────────────────────────────────────────────────
// 🔥 BASE DE DATOS EN MEMORIA (CRON JOB)
// ─────────────────────────────────────────────────────────────
let MEMORY_DB = [];
let isSyncing = false;
...
router.get('/buscar', async (req, res) => {
    try {
        ...
        let esFallback = false; // Bandera de seguridad

        // Fallback de emergencia si la memoria está vacía (Railway recién reiniciado)
        ...
        // 🔥 SOLUCIÓN: Solo aplicamos este filtro si los datos vienen de la RAM. 
        // Si vienen del fallback, Alaluf ya los filtró por nosotros. Volver a filtrarlos vaciaba el arreglo.
        if (!esFallback) {
            if (req.query.tipo_prop) {
                rawDataArray = rawDataArray.filter(p => p.tipo_prop == req.query.tipo_prop || p.id_tipo_prop == req.query.tipo_prop || p.id_tipo == req.query.tipo_prop);
            }
        }
